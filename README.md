...
.venv\Scripts\python scripts\oauth_bootstrap.py C:\path\client_secret.json --label english
.venv\Scripts\python scripts\oauth_bootstrap.py C:\path\client_secret.json --label thai
```

7. Copy only the resulting values into GitHub Secrets, then delete the local output containing tokens.
